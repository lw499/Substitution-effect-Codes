...
myfunc = function(m)
{
  ...
  fitA1 = fit_hal(X = xmatA1, Y = dftmp$A1, family = "binomial",smoothness_orders = 0, max_degree=3)
  ...
  fitA2 = fit_hal(X = xmatA2, Y = dftmp$A2, family = "binomial",smoothness_orders = 0, max_degree=3)
  ...
  df$ypred = predict(yfitup, newdata = ydat, type="response"); # part 1 (added)
  ...
  df$ypred2 = ydat$ypred # part 2 (added)
  
  fitmsm = glm2(ypred ~ L2, family=quasibinomial(), data = ydat) ; 
  df$ypredp2 = predict(fitmsm, newdata=df, type="response") # added
  #myparam = coef(fitmsm)
  #myparam
  
  ## added
  part1 = (I(df$A==0)*df$num_tot)/(df$denom1*df$denom2)*(df$Y-df$ypred)
  part1 = ifelse(df$A==0, part1, 0)
  part2 = df$ypred2 - df$ypredp2
  
  expectedinv = 0; expected1 = 0;
  for(i in 1:n)
  {
    sz = as.matrix(c(1,df$L2[i]), nrow=2) %*% t(as.matrix(c(1,df$L2[i]), nrow=2))
    expected1 = expected1 + sz*((part1[i]+part2[i])^2)
    expectedinv = expectedinv + sz*(df$ypredp2[i]*(1-df$ypredp2[i]))
  }
  expected1_f = expected1/n
  expectedinv_f = solve(expectedinv/n)
  myparamtmp = ((expectedinv_f) %*% expected1_f %*% t(expectedinv_f))
  myparam = sqrt(diag(myparamtmp))/sqrt(n)
  myparam
