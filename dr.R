...
myfunc = function(m)
{
  ...
  ##################
  ydat = tmpdata
  yfito = glm2(Y ~ A+B+C+L, family=binomial(), data = ydat) ; 
  ydat$ypredo = predict(yfito, newdata = ydat)
  yfitup = glm(Y~L, offset = ypredo, weight = (I(A==0)*num_tot)/(denom1*denom2), family=quasibinomial(), data=ydat)
